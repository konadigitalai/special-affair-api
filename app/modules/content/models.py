...
class NavigationItem(TimestampMixin, Base):
    ...
    parent_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("navigation_items.id", ondelete="CASCADE"), index=True
    )
...
class ContentPage(TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint("kind IN ('page', 'story')", name="kind_allowed"),
    )
...
class ConsentRecord(TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint("action IN ('granted', 'withdrawn')", name="action_allowed"),
    )
    ...
    source: Mapped[str] = mapped_column(
        String(100), nullable=False, default="storefront"
    )
    customer_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("customers.id"), index=True
    )
    notice_version: Mapped[str | None] = mapped_column(String(100))
