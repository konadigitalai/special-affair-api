...
class Order(TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('pending_payment','payment_failed','expired','confirmed','allocated','partially_fulfilled','fulfilled','delivered','cancelled')",
            name="status_allowed",
        ),
    )
...
class OrderItem(TimestampMixin, Base):
    ...
    line_tax_minor: Mapped[int] = mapped_column(
        BigInteger, default=0, server_default="0"
    )
    promotion_id: Mapped[UUID | None] = mapped_column(ForeignKey("promotions.id"))
